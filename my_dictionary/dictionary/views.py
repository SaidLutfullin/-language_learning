...
from .serializers import WordsSerializer, WordAnswerSerializer
from common.models import ServicePage
from .forms import AddinWordForm, EditingForm, Exporting
from .words_operation import (get_words_list, get_question, send_answer,
        ...
        if 'save' in self.request.POST:
            if form.cleaned_data['start_learning']:
                ...
                word.save(update_fields=["russian_word", "foreign_word",
                                         "context", "asking_date", "box_number"])
            else:
...
class WordOperations(LoginRequiredMixin, APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            send_answer(request.user, serializer.data['word_id'], serializer.data['is_correct'])
